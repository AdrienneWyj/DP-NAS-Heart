...
from ranpac import RanPACwithOpt

# add 2025-08-26
class BackboneWrapper(nn.Module):
    """把已训练好的 DP-NAS 模型包装为特征提取器。
       优先取 (logits, emb) 里的 emb；否则直接用 logits 作为特征。"""
    def __init__(self, dpnas_model: nn.Module):
        super().__init__()
        self.m = dpnas_model.eval()

    @torch.no_grad()
    def forward(self, x):
        out = self.m(x)
        # 如果 Net 返回 (logits, emb)
...
class Trainer:
    ...
    def train_val_net(self, state_list, dataset, weight_initializer, device, args, save_path):
        ...
        # add 2025-05-08
        # input_dim = dataset.train_features.shape[1]
        # dropout_rate = 0.5  # 你想定多少就写多少
        # model = Net(input_dim=input_dim, num_classes=num_classes, do_drop=dropout_rate)
        ...
        model = Net(
            state_list=state_list,   # 这里就是你函数里传进来的 state_list 参数
            num_classes=num_classes,
            net_input=net_input,
            bn_val=args.batchnorm_value if hasattr(args, 'batchnorm_value') else 0.0,
            do_drop=0.5  # 或你想要的值
        )
        ...
        # add 2025-08-26
        # =====================【ADD RanPAC】DP-NAS → RanPAC 二分类=====================
        run_ranpac = True  # 如需临时关闭，改为 False
        if run_ranpac:
            # 1) 取单卡模型（若使用 DataParallel）
            dpnas_feat_model = model.module if (torch.device('cuda') == device and isinstance(model, torch.nn.DataParallel)) else model
            backbone = BackboneWrapper(dpnas_feat_model).to(device)

            # 2) 自动探测 DP-NAS 输出特征维度
            with torch.no_grad():
                sample_x, _ = next(iter(dataset.train_loader))
                sample_x = sample_x.to(device)
                sample_feat = backbone(sample_x)
                feat_dim = int(sample_feat.shape[1])

            ranpac = RanPACwithOpt(input_shape=feat_dim, num_classes=2, backbone=backbone, M=512, device=str(device))
            ranpac.train_(dataset.train_loader)  # 在线/增量式地学习

            # 4) 评估（优先 test_loader，否则用 val_loader）
            eval_loader = dataset.test_loader if dataset.test_loader is not None else dataset.val_loader
            correct, total = 0, 0
            cm = np.zeros((2, 2), dtype=int)  # 混淆矩阵 rows=true(0/1), cols=pred(0/1)

            ranpac.eval()
            with torch.no_grad():
                for xb, yb in eval_loader:
                    xb = xb.to(device)
                    pred = ranpac.predict(xb).squeeze(1).detach().cpu().numpy()  # [N]
                    y = yb.numpy()
                    correct += (pred == y).sum()
                    total   += y.shape[0]
                    # 更新混淆矩阵
            ...
            acc_ranpac = (correct / float(total)) if total > 0 else 0.0
            print(f'[DP-NAS + RanPAC] ACC = {acc_ranpac:.4f}')
            print('[DP-NAS + RanPAC] Confusion Matrix (rows:true 0/1, cols:pred 0/1):')
            print(cm)

            # 5) 可选：保存 RanPAC（以便下次直接加载）
            try:
                ranpac.save_model(save_path, 'ranpac')
                print(f'[DP-NAS + RanPAC] 模型已保存到: {os.path.join(save_path, "ranpac.pth")}')
            except Exception as e:
                print("[DP-NAS + RanPAC] 保存失败（忽略）：", e)
        # =====================【END RanPAC】===========================================
