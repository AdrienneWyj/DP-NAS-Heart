...
class Net(nn.Module):
    def __init__(self, state_list, num_classes, net_input, bn_val, do_drop):
        ...
        input_dim = net_input.size(1)  # 表格数据输入

        self.gpu_mem_req = 0  # 不估算内存
        ...
        for state_no, state in enumerate(state_list):
            if state.layer_type == 'fc':
                ...
                layers.append(('linear_' + str(state_no), nn.Linear(in_dim, out_dim, bias=True)))
                layers.append(('relu_' + str(state_no), nn.ReLU(inplace=True)))
                current_dim = out_dim
            elif state.layer_type == 'dropout':
                dropout_rate = state.fc_size / 100.0
                layers.append(('dropout_' + str(state_no), nn.Dropout(dropout_rate)))
            elif state.layer_type == 'terminate':
                break

        # 最后加一个分类层
        layers.append(('output', nn.Linear(current_dim, num_classes)))
