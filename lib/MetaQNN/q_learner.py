...
class QLearner:
    ...
    def add_search_net_to_replay_buffer(self, search_net_in_replay_buffer, reward=None,
        ...
        new_row = pd.DataFrame([[self.epsilon,
                         self.state_string,
                         train_flag,
                         reward,
                         acc_best_val,
        ...
        self.replay_buffer = pd.concat([self.replay_buffer, new_row], ignore_index=True)
    ...
    def _update_q_value(self, start_state, to_state, reward):
        ...
        # 检查 action 是否在 actions 里，防止找不到时报错
        if action_between_states not in actions:
            print(f"⚠️ Warning: action {action_between_states} not found in actions list for state {start_state.as_tuple()}. Skipping this update.")
            return  # 跳过这次更新

        # Q-learning iterative Bellman update
        idx = actions.index(action_between_states)
        values[idx] = values[idx] + \
            self.args.q_learning_rate * (reward + self.args.q_discount_factor * max_over_next_states - values[idx])
