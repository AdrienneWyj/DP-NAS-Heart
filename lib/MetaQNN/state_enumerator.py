...
class StateEnumerator:
    ...
    def enumerate_state(self, state, q_values):
        """
        defines all possible state transitions for MLP-style search
        """
        actions = []

        if state.terminate == 0:
            # 允许添加 Linear 层
            linear_layer_sizes = [32, 64, 128, 256]  # 可以自定义更多

            for fc_size in linear_layer_sizes:
                actions.append(State(layer_type='fc',
            ...
            for dropout_rate in dropout_rates:
                # 这里直接用 fc_size 暂存 dropout比例（因为原State没有专门的dropout参数，可以稍后在network里解释）
                actions.append(State(layer_type='dropout',
                                    layer_depth=state.layer_depth + 1,
                                    filter_depth=0,
                                    filter_size=0,
                                    stride=0,
                                    padding=0,
                                    image_size=0,
                                    fc_size=int(dropout_rate * 100),  # 0 or 50，后面再解释
                                    terminate=0))

            # 允许终止网络
            actions.append(State(layer_type='terminate',
                                layer_depth=state.layer_depth + 1,
                                filter_depth=0,
                                filter_size=0,
                                stride=0,
